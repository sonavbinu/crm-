...
const State = () => {
  ...
  const handleDelete = (indexDelete) => {
    const updated = state.filter((_, index) => index !== indexDelete);
  ...
  const handleEdit = (index) => {
    ...
    setCode(state[index].code);
    setEdit(index);
    setShowForm(true);
  };

  const confirmDelete = () => {
    const updated = state.filter((_, index) => index !== deleteIndex);
    setState(updated);
    localStorage.setItem("state", JSON.stringify(updated));
  ...
  return (
    <div>
      ...
      <div className="flex  flex-col items-end p-2 mt-3 gap-10">
        <div>
          <button
            onClick={() => {
              setShowForm(true);
